<script lang="ts">
  ...
  import {getTemplate, addPattern} from "scripts/getters";
...
<g clip-path="url(#shield_{id})">
  ...
  {#if division && division.division !== "no"}
    ...
    {#each ordinariesRegular as ordinary, i}
      {#if ordinary.divided === "field" || ordinary.divided === "counter"}
        <Ordinary {coa} {ordinary} {i} {shieldPath} counterTincture={division.t} {type} />
      {/if}
    {/each}
    ...
    {#each charges as charge, i}
      {#if charge.divided === "field" || charge.divided === "counter"}
        <Charge {coa} {charge} {i} shield={$shield} counterTincture={division.t} {type} />
      {/if}
    {/each}
    ...
    <g class="division" clip-path="url(#division_{id})">
      ...
      {#each ordinariesRegular as ordinary, i}
        {#if ordinary.divided === "division" || ordinary.divided === "counter"}
          <Ordinary {coa} {ordinary} {i} {shieldPath} counterTincture={coa.t1} {type} />
        {/if}
      {/each}
      ...
      {#each charges as charge, i}
        {#if charge.divided === "division" || charge.divided === "counter"}
          <Charge {coa} {charge} {i} shield={$shield} counterTincture={coa.t1} {type} />
        {/if}
      {/each}
  ...
  {#each ordinariesRegular as ordinary, i}
    {#if !ordinary.divided}
      <Ordinary {coa} {ordinary} {i} {shieldPath} {type} />
    {/if}
  {/each}
  ...
  {#each charges as charge, i}
    {#if !charge.divided || !division}
      <Charge {coa} {charge} {i} shield={$shield} {type} />
    {/if}
  {/each}
