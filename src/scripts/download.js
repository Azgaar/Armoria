...
export async function download(i, format = "png") {
  const coas =
    i || i === 0
      ? [document.getElementById("coa" + i)]
      : document.querySelectorAll("svg.coa");
...
async function getURL(svg, width, height) {
  ...
  const gr = get(grad),
    di = get(diaper);
  if (gr && gr !== "no")
    d.insertAdjacentHTML("beforeend", defs.getElementById(gr).outerHTML);
  if (di && di !== "no")
    d.insertAdjacentHTML("beforeend", defs.getElementById(di).outerHTML);
  ...
  const fieldPattern = clone
    .getElementsByClassName("field")[0]
    .getAttribute("fill")
    .split("(#")[1]
    ?.split(")")[0];
  if (fieldPattern) addPattern(fieldPattern, d);
  const divisionPattern = clone
    .getElementsByClassName("division")[0]
    ?.querySelector("rect")
    .getAttribute("fill")
    .split("(#")[1]
    ?.split(")")[0];
  ...
  function addPattern(pattern, d) {
    ...
    d.insertAdjacentHTML(
      "beforeend",
      document.getElementById(pattern).outerHTML
    );
    ...
    if (pattern.slice(0, 4) === "semy") {
      ...
      if (!addedElements[charge]) {
        d.insertAdjacentHTML(
          "beforeend",
          document.getElementById(charge).outerHTML
        );
        addedElements[charge] = true;
      }
    }
