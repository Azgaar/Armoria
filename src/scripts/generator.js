...
export const generate = function(seed = Math.floor(Math.random() * 1e9)) {
    ...
    if (division) {
        ...
        if (divisions[division]) coa.division.line = config.usedPattern || (config.ordinary && P(.7)) ? "straight" : rw(divisions[division]);
    }
        ...
        if (p === "ABCDEFGHIKL" && P(.95)) {
            ...
            if (P(.3) && ["perPale", "perFess"].includes(division) && coa.line === "straight") {
                ...
                const [p1, p2] = division === "perPale" ? ["p", "q"] :
                    ...
                    division === "perBend" ? ["l", "m"] : ["j", "o"]; // perBendSinister
                coa.charges[0].p = p1;
...
function getTincture(config, element, fields = [], RoT) {
    ...
    while (tincture === base || fields.includes(tincture)) { tincture = rw(config.tData[type]); } // follow RoT
