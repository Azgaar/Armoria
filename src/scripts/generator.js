...
export const generate = function (seed = Math.floor(Math.random() * 1e9)) {
  ...
  const linedOrdinary =
    (charge && P(0.3)) || P(0.5) ? rw(ordinaries.lined) : null;
  config.ordinary =
    (!charge && P(0.65)) || P(0.3)
      ? linedOrdinary
        ? linedOrdinary
  ...
  if (division) {
    const t = getTincture(
      config,
      "division",
      config.usedTinctures,
      P(0.98) ? coa.t1 : null
    );
    coa.division = { division, t };
    if (divisions[division])
      coa.division.line =
        config.usedPattern || (config.ordinary && P(0.7))
          ? "straight"
          : rw(divisions[division]);
  }
  ...
  if (config.ordinary) {
    ...
    if (linedOrdinary)
      coa.ordinaries[0].line =
        config.usedPattern || (division && P(0.7)) ? "straight" : rw(lines);
    if (
      division &&
      !charge &&
      !config.usedPattern &&
  ...
  if (charge) {
    ...
    if (positions.ordinariesOn[config.ordinary] && P(0.8)) {
      ...
      while (charges.natural[charge] === ordinaryT)
        charge = selectCharge(config);
      t =
        !config.usedPattern && P(0.3)
          ? coa.t1
          : getTincture(config, "charge", [], ordinaryT);
      ...
      t =
        !config.usedPattern && P(0.3)
          ? ordinaryT
          : getTincture(config, "charge", config.usedTinctures, coa.t1);
      ...
      t = getTincture(
        ...
        "charge",
        ordinaryT
          ? config.usedTinctures.concat(ordinaryT)
          : config.usedTinctures,
        coa.t1
      );
    ...
    if (p === "ABCDEFGHIKL" && P(0.95)) {
      ...
      const allowCounter =
        !config.usedPattern && (!coa.line || coa.line === "straight");
      ...
      if (
        ...
        if (P(0.95)) {
          const p2 =
            p === "e" || P(0.5) ? "e" : rw(positions.divisions[division]);
          const charge = selectCharge(charges.single);
          const t = getTincture(
            config,
            "charge",
...
function selectCharge(config, set) {
  const type = set
    ? rw(set)
    : config.ordinary || config.divisioned
    ? rw(charges.types)
    : rw(charges.single);
...
function getTincture(config, element, fields = [], RoT) {
  ...
  if (RoT && type !== "patterns")
    type = getType(config, base) === "metals" ? "colours" : "metals"; // follow RoT
  ...
  if (type !== "patterns" && element !== "charge")
    config.usedTinctures.push(tincture); // add field tincture
