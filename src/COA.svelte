<script>
  ...
  $: strokeWidth = borderWidth * (edit ? 1 : 2);

  function click() {alert(JSON.stringify(coa, null, "    "));}
...
<svg id="coa{i}" class="coa" xmlns="http://www.w3.org/2000/svg" width={w} height={h} viewBox="0 0 200 200" on:click={click}>
  <defs>
    {#if division}
  ...
  <g id="shield" clip-path="url(#{shield})">
    ...
    {#if ordinary?.counter}<Ordinary {ordinary} {shieldPath} {colors} t={coa.t3}/>{/if}
    {#if ordinary?.crop}<Ordinary {ordinary} {shieldPath} {colors} t={coa.t2}/>{/if}
    ...
    {#each charges as charge, i}
      {#if charge.type === "field"}
        <Charge {charge} {i} {shield} {colors} t={charge.t}/>
      {:else if charge.type === "counter"}
        <Charge {charge} {i} {shield} {colors} t={coa.t3}/>
      {/if}
    {/each}
    ...
    <!-- overall layer -->
    {#if ordinary && !ordinary.crop && !ordinary.counter}<Ordinary {ordinary} {shieldPath} {colors} t={coa.t2}/>{/if}
    {#if diaperType === "overall"}<rect class="diaper" x=0 y=0 width=200 height=200 fill="url(#{diaper})"/>{/if}
    {#each charges as charge, i}{#if !charge.type}<Charge {charge} {i} {shield} {colors} t={charge.t}/>{/if}{/each}
  </g>
